...
At momentum scales where the strong coupling $\alpha_s$ becomes large ($\alpha_s \approx \mathcal{O(1)}$), QCD process can not longer be calculated using perturbation theory and instead lattice QCD~\cite{PhysRevD.10.2445,DeGrand:1055545} is used. 
...
\end{equation}
with one crucial difference---the omission of the fermion masses. As will be illustrated in \cref{sec:ssb}, mass terms cannot be included in the electroweak Lagrangian without breaking gauge symmetry. For left-handed fermion fields, local $SU(2)_\mathrm{L}$ transformations can be written as
\begin{equation}
...
By modifying the covariant derivative to include a $U(1)_Y$ gauge field and ensuring that $U(1)_Y$ acts the same on left- and on right-handed fermions it becomes $\codiff_\mu &= \partial_\mu + ig_2\frac{\sigma^a}{2}W^a_\mu + ig_1\frac{Y}{2}B_\mu$ for left-handed fermions and $\codiff_\mu &= \partial_\mu + ig_1\frac{Y}{2}B_\mu$ for right-handed fermions. The the full electroweak Lagrangian thus reads
\begin{equation}
	\Lagr_\mathrm{electroweak} = \bar{\psi}i\gamma^\mu\codiff_\mu\psi - \frac{1}{4}W^a_{\mu\nu}W^{\mu\nu,a} - \frac{1}{4}B_{\mu\nu}B^{\mu\nu},
\end{equation}
where a term describing the kinematics of the gauge field $B_\mu$ has already been added and $B_{\mu\nu} = \partial_\mu B_\nu - \partial_\nu B_\mu$. 
