...
Although revealing a good agreement, the CL$_s$ values naturally do not exactly match. For this reason, the simplified likelihood can only be used for models giving observed CL$_s$ moderately far away from the exclusion boundary at 0.05. Models with a CL$_s$ value too close to 0.05 need to be evaluated using the full likelihood and \textsc{Recast} for full statistical precision. Compared to the single-bin approach using the discovery signal regions, the benefit of the simplified likelihood is that, due to the improved agreement in observed CL$_s$, the interval around CL$_s=0.05$ defining models to be evaluated using the full likelihood can be chosen to be significantly narrower. Ultimately, this approach does allows for more efficient \gls{pmssm} scans where a smaller fraction of models needs to be evaluated at full precision.
\begin{figure}[H]
...
\Cref{fig:lsp_phenomenology_parameters} shows the \gls{lsp} type as a function of the \gls{pmssm} parameters $M_1$, $M_2$, $\mu$ and $\tan\beta$. Models with $\vert M_1\vert \ll \vert M_2\vert, \vert\mu\vert$ tend to have an \gls{lsp} with dominant bino component, while models with $\vert M_2 \vert \ll \vert M_1 \vert, \vert\mu\vert$, have an \gls{lsp} that is mostly wino-like.
