...
In the bulk of the $m(\charg)$--$m(\lsp)$ plane, \ie the parameter space targeted by the \onelepton search using the simplified model, the large majority of the models produce a bino-like \gls{lsp} with nearly mass-degenerate $\charg$ and $\neutr$. These models correspond to cases where $M_1 \ll M_2, \mu$ and are closest to the canonical simplified model considered in the \onelepton search. Some sensitivity can be expected towards these models, provided that the branching fractions of the decays $\neutr \rightarrow h \lsp$ and $\charg \rightarrow W^\pm \lsp$ are large enough and produce on-shell bosons.
...
The models excluded by the \onelepton search can roughly be classified in two categories: models lying within the simplified model exclusion contour and models with nearly mass-degenerate $\charg$ and $\neutr$. All models excluded within the simplified model exclusion contour turn out to be models where the masses of the $\charg$ and $\neutr$ are not too far apart (but no longer required to be strictly degenerate) and the \gls{lsp} is bino-like. This behavior can clearly be seen in \cref{fig:impact_electroweakinos_2D_bino_lsp} where only models with a bino-like \gls{lsp} are shown and similar $\charg$ and $\neutr$ mass ranges are excluded. Thus, the \onelepton search is most sensitive to $\charg\neutr$ production with wino-like electroweakinos and a bino-like $\lsp$, corresponding to models with a spectrum close to that of the canonical simplified model signature originally considered in the search. In general however, the sensitivity to \gls{pmssm} models is significantly reduced compared to the simplified model exclusion contour, even in the parameter space where most models produce a bino-like \glspl{lsp} and have nearly mass-degenerate $\charg$ and $\neutr$, generating models that should be close to the simplified model. The crucial difference, responsible for the loss in sensitivity, is the fact that the simplified model assumes branching ratios of 100\% of the $\neutr \rightarrow h \lsp$ decay, an assumption that is in general not realised in the \gls{pmssm} models. \Cref{fig:mchi1p_mchi20_contour_bino_lsp_wh_only} illustrates the impact of mixed branching fractions by artificially restoring the simplified model assumption in the \gls{pmssm} models, resulting in significantly more models being excluded within the simplified model contour.   
