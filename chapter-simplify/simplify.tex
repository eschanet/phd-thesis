...
This chapter therefore introduces the concept of \textit{simplified likelihoods}, a method approximating the statistical model of an analysis using the \lib{HistFactory} template in order to achieve more efficient profile likelihood fits and, ultimately, hypothesis tests.
...
One of the main figures of merit of an analysis approximation is naturally the reduction in computational wall time compared to the full analysis. \Cref{fig:benchmark_1Lbb} shows a benchmark for different likelihood configurations in the context of the \onelepton search.
...
In summary, this demonstrates that the method can, in many cases, offer a fast and reliable approximation of ATLAS searches for \gls{susy} using the \lib{HistFactory} template.
