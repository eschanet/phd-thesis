...
\Cref{lst:bkg_sample} shows a representative total background sample, called `\texttt{total\_bkg}', defining expected event rates for a channel with three bins. The single nuisance parameter is called `\texttt{total\_error}' and is implemented as a \textit{rate modifier} (cf. \cref{sec:likelihood_function}).
