...
\end{equation}
where $N^\mathrm{MC}_p(\mathrm{R}_j)$ and $N^\mathrm{MC}_p(\mathrm{CR}_i)$ are the expected event rates for the process $p$ in CR$_i$ and R$_j$, respectively. The systematic uncertainty on the transfer factor $\upDelta f_p^\mathrm{sys}$ is then given by
\begin{equation}
	\upDelta f_p^\mathrm{sys} = \frac{f_p^\mathrm{var}}{f_p^\mathrm{nom}} - 1,
\end{equation}
with $f_p^\mathrm{var}$ and $f_p^\mathrm{nom}$ the transfer factors from the variational and nominal samples, respectively.
...
\begin{equation}
	\upDelta f_p^\mathrm{stat} = (\upDelta f_p^\mathrm{sys} + 1 ) \sqrt{\sum_{n\in\makemebold{N}}(\frac{\sigma_n}{n})^2},
\end{equation}
 ...
 \begin{equation}
 	\upDelta n_p^\mathrm{sys}(\mathrm{R}_i) = \frac{n_p^\mathrm{sys}(\mathrm{R}_i)n_p^\mathrm{nom}(\mathrm{P})}{n_p^\mathrm{nom}(\mathrm{R}_i)n_p^\mathrm{sys}(\mathrm{P})} - 1,
 \end{equation}
