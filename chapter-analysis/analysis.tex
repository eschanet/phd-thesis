...
\Cref{fig:ttbar} shows an example of a decay of a $\ttbar$ system into a final state with a lepton, multiple jets (two of which originate from \textit{b} quarks) and missing transverse momentum.
...
%	\end{subfigure}
	\caption{Representative Feynman diagrams showing the dominant processes \subref{fig:ttbar} $t\bar{t}$, \subref{fig:singletop} single top and \subref{fig:wjets} $W+\textrm{jets}$ production with subsequent decays.}
...
\begin{table}
	...
	\midrule
	$V+\mathrm{jets}$ & \multicolumn{2}{c}{\textsc{Sherpa} 2.2.1} & NNPDF 3.0 NNLO~\cite{Ball:2014uwa} & NNLO~\cite{Anastasiou:2003ds} & \textsc{Sherpa} default \\
